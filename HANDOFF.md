...
- 원인: `static_submenu_tables.translate_ui` 가 UI 표 문자열을 통째로 풀어 구절 치환했다. 45·47·49번의 `F9 04 00 61`(printItemAmount item=0x61)에서 0x61 이 "D" 로 읽혀 "Dコ"→"개"(`F9 FC 12 00`)에 걸렸다. 결과는 `F9 04 00 F9` + `FC 12 00`, 즉 save(jumpIfSuccessful=18) 명령이다.
- 그래서 게임이 저장한 뒤 18번(내비 칩 8장 제한 문구)으로 점프하고, 선택지를 기다리던 상태 화면이 멈춘다. 세이브도 그 순간 한 번 써진다(버스터 UP 은 미사용 상태로 남음).
- 보관 ROM 35개(v0.9.3~v0.9.24)가 전부 같은 바이트이고 v0.9.25 만 정상이다. 치환은 커밋 58c4c9a(V0.9.4)에서 들어왔다.
- 수정 1: `ui_tokens` 는 TextPet `mmbn2.ini` 에서 명령 base/mask 를 읽어(select 는 둘째 바이트가 길이) 문자열을 명령과 글자 덩어리로 자르고, 글자만 번역한다. 숫자 행(E9 3바이트 필드)은 기존대로 둔다.
- 수정 2: 빈 엔트리는 비워 둔다. v0.9.4~24 는 E7 을 채워 넣어, 그 번호를 부르면 다음 스크립트 대신 바로 끝났다. waitHold 로 끝나는 엔트리 뒤에 E7 을 덧붙이던 것도 없앴다.
- 수정 3: 가타카나 예외 목록을 지우고 일본어 검사를 엄격하게 바꿨다. 조사 오류 12곳은 통문장 표 `UI_RUNS` 로 고쳤고, 동적 이름 뒤에서는 조사를 빼는 문형으로 썼다.
- 게이트: `verify_submenu_rendering_content.py` 가 원문과 번역본의 네 표를 TextPet 으로 다시 읽어 스크립트별 명령 골격과 빈 칸 위치를 비교한다. v0.9.24 에 돌리면 실패한다(음성 대조). v0.9.25 는 UI 109·설명 255·이름 255+255 전부 일치한다.
- 결과: `poc/output/exe2_rev1_kr_v0_9_25_buster.gba`, SHA-256 `bd9f83ce8b703fd659f6df1f98937759a8889cbc7e5dcd27d6deaf1311eb5ac6`. v0.9.24 와 다른 바이트는 `0x940000~` UI 표 영역뿐이다. 정적·내용 QA 모두 `PASS (bench)`.
- 런타임(Mesen, 에이전트가 캡처 판독), 모두 멈춤 없음:
  - ATTACK 확인 → 예 → "공격력이 올랐어!" → 메뉴 복귀
  - RAPID(남은 2개)에서 B 로 취소
  - CHARGE 강화, 마지막 1개 소진 후 상태 화면 복귀
  - 결함 때 자동 저장된 세이브를 불러 강화
  - 세이브 무변경
- 확인 못 한 것: 버스터 UP 0개에서 A 는 상태 화면에 머물러 "버스터 업이 없어…" 경로에 닿지 않았다.
- 배포 파일(로컬, 미게시):
  - `dist/EXE2_Rev1_KR_V0.9.25_BusterFix.zip`: 932,370 B, `8c0e69e17b6ad776a9ad5d811a4de32d7cc25f82eaafcd213e3895cf4305bf66`
  - `dist/EXE2_Rev1_KR_V0.9.25_BusterFix.xdelta`: 408,705 B, `8e24acfa3fa3f6191e1590d85a3904e1bb42f594d3f789fdfed61afc01d6211e`
- 사용자 요청("릴리즈 해주고 github에도 올려줘")으로 v0.9.25 프리릴리스로 게시한다. 노트 원본: `analysis/github_release_v0925_notes.md`. 기존 릴리스(V0.9.4~V0.9.9, V0.9.24)는 손대지 않는다. 내릴지 경고만 붙일지는 사용자 결정 대기.
