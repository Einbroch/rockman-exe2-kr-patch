#!/usr/bin/env python3
"""Build a non-distributable runtime probe for F1 selector 0x04.

The probe uses a separate ROM copy. It places one Korean glyph in an existing
E5 bank record and replaces five direct glyph bytes in the verified active
entry with an F1 test frame followed by that E5 glyph. The entry length,
pointers, parser code, and original ROM remain unchanged.
"""
...
# F1 selector 0x04 has a bounded static r4-delta candidate of three bytes.
# The third byte is a zero-valued reserved filler; the E5 sequence starts at
# the predicted next cursor location. This frame is only a runtime probe.
PROBE_FRAME = bytes((0xF1, 0x04, 0x00, 0xE5, FONT_RECORD_INDEX - 0xE5))
...
def main() -> None:
    ...
    preview = args.preview_dir / f"glyph_U+{ord(GLYPH):04X}.png"
    ...
    expected_writes = [
        ...
        {
            "kind": "length_preserving_f1_cursor_probe",
    ...
    manifest = {
        "schema_version": 1,
        "kind": "non_distributable_f1_cursor_runtime_probe",
        ...
        "expected_screen_result": "the Korean glyph at the probe position appears after F1 selector 0x04",
        "limitations": [
            "A visible glyph supports only this selector's cursor-placement hypothesis in this entry.",
            "It does not identify the command, establish all F1 selector values, or authorize script reinsertion.",
