...
VERSION='V0.9.4-MenuTextFix'
QA_PREFIX='exe2_rev1_menu_textfix'
...
def main():
    ...
    static=ROOT/'analysis'/f'{QA_PREFIX}_static_qa.json'
    content=ROOT/'analysis'/f'{QA_PREFIX}_content_qa.json'
    runtime=ROOT/'analysis'/f'{QA_PREFIX}_runtime_review.json'
    ...
    fit=None
    if VERSION.startswith('V0.9.5'):
        fit_path=ROOT/'analysis/exe2_rev1_ui_fit_regression_qa.json'
        fit=json.loads(fit_path.read_text(encoding='utf-8'))
        assert fit['status']=='PASS_UI_FIT_REGRESSION' and fit['candidate_sha256']==candidate_sha
        for item in fit['comparison_images']:
            assert sha(ROOT/item['path'])==item['sha256']
    ...
    # Tool source is a pinned dependency, not an incidental member of an old
    # distribution ZIP which the user may legitimately archive or remove.
    flips_source=ROOT/'external/Flips-v198/flips-source-v198.zip'
    assert sha(flips_source)=='194a5f061c6c82d8d62704781bc1e93c0239fcbe044a4511b1ce7221457b7617'
    with zipfile.ZipFile(flips_source) as archive:assert archive.testzip() is None
    shutil.copy2(flips_source,folder/'tools/flips-source-v198.zip')
    ...
    manifest={'version':VERSION,'status':'EMULATOR_VALIDATION_CANDIDATE',
              ...
              'messages':{'success':VERSION+' 메뉴 표시 수정 패치 적용 완료.',
                          'note':'새 ROM을 완전히 다시 실행하고 게임 내 일반 저장을 사용하세요.'}}
    (folder/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    text='# EXE2 Rev 1 한국어 패치 '+VERSION+'\n\n'+(
        ...
        'V0.9.5 UiFit: 두 주황색 화살표의 메모/라이브러리는 원래 P.A와 같은 높이의 7×6 픽셀 글자를 사용합니다. '
        '기존 글자와 그림자 잔여물을 제거하고 아래쪽 안쪽 여백을 보존했습니다. '
        '저장 항목은 8칸 폭에 맞춰 데이터라이브러리로 붙여 써 마지막 리까지 표시합니다.\n\n'
    ...
    if fit is not None:receipt['ui_fit_regression_sha256']=sha(fit_path)
    ...
    (ROOT/'analysis'/f'{QA_PREFIX}_package_qa.json').write_text(json.dumps(receipt,indent=2)+'\n',encoding='utf-8')
...
if __name__=='__main__':
    ...
    p.add_argument('--qa-prefix',default=QA_PREFIX)
    a=p.parse_args()
    TARGET=a.target.resolve();NAME=a.name;VERSION=a.version;QA_PREFIX=a.qa_prefix
    assert all('/' not in x and '\\' not in x for x in (NAME,QA_PREFIX))
    main()
