...
try {
    ...
    if ((Get-Item -LiteralPath $sourcePath).Length -ne $manifest.source.size -or
        ...
        throw 'Wrong source ROM. Use the unpatched Japan Rev 1 ROM; see README_KO.txt.'
    }
    if (-not $OutputRom) { $OutputRom = Join-Path $PSScriptRoot 'Rockman_EXE2_Rev1_KR_TEST.gba' }
    ...
    $patchPath = Join-Path $PSScriptRoot 'EXE2_Rev1_KR_TEST.bps'
    ...
    Write-Host 'Verified: exact Korean TEST ROM. Not a finished release.'
    Write-Host 'Open it in mGBA with audio muted. Do not load save states from older ROMs.'
    exit 0
} catch {
