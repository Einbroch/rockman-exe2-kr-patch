#!/usr/bin/env python3
"""Build a length-preserving cursor-boundary probe for F9 selector 0x00.

The Korean marker begins at the fourth byte of a five-byte replacement frame.
If the observed F9:00 path consumes four stream bytes, the marker prefix is
consumed as frame data and is not rendered. A visible marker instead refutes
that four-byte cursor position at this entry.
"""
...
def main() -> None:
    ...
    probe_frame = bytes((0xF9, 0x00, 0x00, 0xE5, FONT_RECORD_INDEX - 0xE5))
    ...
    expected_writes = [
        ...
        {
            "kind": "length_preserving_f9_00_cursor_probe",
            ...
            "opcode": 0xF9,
            "selector_value": 0x00,
            "marker_transport": "E5 starts at the fourth frame byte",
    ...
    manifest = {
        "schema_version": 1,
        "kind": "non_distributable_f9_00_cursor_runtime_probe",
        ...
        "expected_screen_result": "An absent Korean marker with normal dialogue supports this F9:00 four-byte cursor candidate; a visible marker refutes it.",
        "limitations": [
            "This is one selector and one entry only.",
            "It does not identify F9, establish its state effect, or authorize command changes.",
